...
def calculate_position_size(
    ...
    """
    Sinyal gücü + volatilite bazlı dinamik pozisyon büyüklüğü hesaplar.
    ...
    if atr is None or atr <= 0 or price <= 0:
        notional = portfolio_value * 3.0 / 100
        log.info(f"ATR/fiyat verisi yok — sabit %3 uygulandı: ${notional:.0f}")
        return notional
    ...
    log.info(
        ...
        f"effective={effective_kelly:.3f} → %{position_pct:.1f} = ${notional:.0f}"
    )
    return notional
